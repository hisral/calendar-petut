...
// --- HTML LAYOUT (RESPONSIVE UPDATE) ---
const Layout = (content: any, title: string, user?: any) => html`
<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"> <!-- Prevent Zoom on Mobile -->
  <title>${title}</title>
...
<body class="bg-gray-100 font-sans h-screen flex flex-col md:flex-row overflow-hidden text-slate-800">
  ...
  <!-- 2. MOBILE BOTTOM NAV (Visible only on Mobile) -->
  <nav class="md:hidden fixed bottom-0 w-full bg-white border-t border-gray-200 flex justify-around items-center h-16 z-40 pb-safe">
    ...
    <a href="/cashflow" class="flex flex-col items-center justify-center w-full h-full ${title === 'Buku Kas' ? 'text-emerald-600' : 'text-gray-400'}">
      ...
      <span class="text-[10px] font-medium mt-1">Kas</span>
    </a>
    ${user.role === 'admin' ? html`
    ...
    <div class="flex items-center justify-center h-full w-full bg-[#f8fafc] px-4">
      <div class="w-full max-w-sm p-6 bg-white rounded-2xl shadow-xl border border-slate-100">
        <div class="text-center mb-6">
            <h1 class="text-2xl font-bold text-slate-800">Team App</h1>
            <p class="text-slate-500 text-xs mt-1">Login untuk akses sistem</p>
        </div>
        <form action="/login" method="post" class="space-y-4">
          <div><label class="block text-slate-700 text-xs font-bold mb-1">Username</label><input type="text" name="username" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm" required></div>
          <div><label class="block text-slate-700 text-xs font-bold mb-1">Password</label><input type="password" name="password" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none text-sm" required></div>
          <button class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl shadow-lg transition transform active:scale-95 text-sm" type="submit">Masuk</button>
        </form>
      </div>
    </div>
  `, 'Login'));
...
app.post('/login', async (c) => {
  ...
  if (!user || (await hashPassword(String(password))) !== user.password) {
     return c.html(Layout(html`<div class="h-full flex items-center justify-center flex-col gap-4 px-4 text-center"><div class="text-red-500 text-lg font-bold">Username / Password Salah</div><a href="/" class="text-blue-600 hover:underline bg-blue-50 px-4 py-2 rounded-lg">Coba Lagi</a></div>`, 'Error'));
  }
...
// 2. DASHBOARD (CALENDAR RESPONSIVE)
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded',function(){
        calendar=new FullCalendar.Calendar(document.getElementById('calendar'),{
          // Change View based on Device
          initialView: isMobile ? 'listWeek' : 'dayGridMonth', 
          headerToolbar: isMobile 
            ? { left: 'prev,next', center: 'title', right: '' } // Simpler toolbar on mobile
      ...
      <div class="bg-white border-b border-gray-200 px-4 md:px-8 py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm">
        <div class="w-full md:w-auto flex justify-between items-center">
            <div>
              ...
              <p class="text-slate-500 text-xs">Pemasukan & Pengeluaran</p>
            </div>
            <!-- Mobile Balance (Visible only on small screens) -->
            <div class="md:hidden text-right">
                <span class="text-[10px] text-gray-500 uppercase font-bold">Saldo</span>
                <div class="text-lg font-bold ${runningBalance >= 0 ? 'text-emerald-600' : 'text-red-600'}">${formatRupiah(runningBalance)}</div>
            </div>
        </div>
        ...
        <div class="w-full md:w-auto flex flex-col md:flex-row gap-3 items-end md:items-center">
            ...
            <div class="hidden md:block text-right mr-4">
                ...
                <div class="text-3xl font-bold ${runningBalance >= 0 ? 'text-emerald-600' : 'text-red-600'}">${formatRupiah(runningBalance)}</div>
            </div>
            <button onclick="document.getElementById('cashModal').classList.remove('hidden')" class="w-full md:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg font-medium shadow-md transition flex justify-center items-center gap-2 active:bg-emerald-800">
              <span>+</span> Transaksi
            </button>
        </div>
      </div>
         ...
         <div class="md:rounded-xl shadow border-t md:border border-gray-200 overflow-x-auto bg-white flex-1">
            <table class="w-full text-left border-collapse min-w-[600px] md:min-w-full">
                <thead class="bg-slate-100 text-slate-600 uppercase text-[10px] md:text-xs font-bold sticky top-0 z-10">
                    <tr>
                        <th class="px-4 md:px-6 py-3 border-b">Tanggal</th>
                        <th class="px-4 md:px-6 py-3 border-b">Keterangan</th>
                        <th class="px-4 md:px-6 py-3 border-b text-right text-emerald-600">Masuk</th>
                        <th class="px-4 md:px-6 py-3 border-b text-right text-red-600">Keluar</th>
                        <th class="px-4 md:px-6 py-3 border-b text-right">Saldo</th>
                        <th class="px-4 md:px-6 py-3 border-b text-center">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 text-xs md:text-sm bg-white">
                    ${data.length === 0 ? html`<tr><td colspan="6" class="p-8 text-center text-gray-400">Belum ada transaksi</td></tr>` : ''}
                    ...
                    <tr class="hover:bg-slate-50 transition group">
                        ...
                        <td class="px-4 md:px-6 py-3 text-slate-600 max-w-[150px] truncate">${item.description}</td>
                        <td class="px-4 md:px-6 py-3 text-right font-medium text-emerald-600">${item.amount > 0 ? formatRupiah(item.amount) : '-'}</td>
                        <td class="px-4 md:px-6 py-3 text-right font-medium text-red-600">${item.amount < 0 ? formatRupiah(Math.abs(item.amount)) : '-'}</td>
                        <td class="px-4 md:px-6 py-3 text-right font-bold text-slate-700 bg-slate-50/50">${formatRupiah(item.balance)}</td>
                        <td class="px-4 md:px-6 py-3 text-center">
                            <button onclick="deleteCash(${item.id})" class="text-red-400 hover:text-red-600 md:opacity-0 group-hover:opacity-100 transition p-2">🗑️</button>
                        </td>
                    </tr>
                    `)}
                </tbody>
    ...
    <div id="cashModal" class="hidden fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-end md:items-center justify-center p-0 md:p-4">
        <div class="bg-white rounded-t-2xl md:rounded-2xl shadow-2xl w-full md:max-w-md animate-slide-up md:animate-none">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-2xl">
                ...
                <button onclick="document.getElementById('cashModal').classList.add('hidden')" class="text-slate-400 text-2xl p-2">&times;</button>
            </div>
            <form action="/api/cashflow" method="POST" class="p-6 space-y-4">
                <div><label class="block text-xs font-bold text-slate-500 uppercase mb-1">Tanggal</label><input type="date" name="date" class="w-full border rounded-lg p-3 text-sm bg-slate-50" required value="${new Date().toISOString().split('T')[0]}"></div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Tipe</label>
                    <div class="grid grid-cols-2 gap-3">
                        <label class="cursor-pointer"><input type="radio" name="type" value="IN" class="peer sr-only" checked><div class="text-center py-3 rounded-lg border border-slate-200 peer-checked:bg-emerald-100 peer-checked:border-emerald-500 peer-checked:text-emerald-700 font-bold transition text-sm">💰 Masuk</div></label>
                        <label class="cursor-pointer"><input type="radio" name="type" value="OUT" class="peer sr-only"><div class="text-center py-3 rounded-lg border border-slate-200 peer-checked:bg-red-100 peer-checked:border-red-500 peer-checked:text-red-700 font-bold transition text-sm">💸 Keluar</div></label>
                    </div>
                </div>
                <div><label class="block text-xs font-bold text-slate-500 uppercase mb-1">Keterangan</label><input type="text" name="description" placeholder="Contoh: Beli Snack" class="w-full border rounded-lg p-3 text-sm outline-none" required></div>
                <div><label class="block text-xs font-bold text-slate-500 uppercase mb-1">Jumlah</label><input type="number" name="amount" placeholder="0" class="w-full border rounded-lg p-3 text-sm outline-none font-mono" required min="1"></div>
                <div class="pt-2"><button class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-lg shadow-md transition text-sm">Simpan</button></div>
            </form>
        </div>
    </div>
