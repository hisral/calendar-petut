...
app.get('/calendar', async (c) => {
  ...
  return c.html(Layout(html`
    ...
    <script>
      ...
      async function deleteEvent(id){ if(confirm('Hapus event ini?')) await fetch('/api/events/'+id,{method:'DELETE'}).then(r=>{if(r.ok){calendar.getEventById(id).remove();closeModal()}else alert('Gagal')}) }
      ...
      document.addEventListener('DOMContentLoaded',function(){
        calendar=new FullCalendar.Calendar(document.getElementById('calendar'),{
          ...
          eventDidMount:function(i){
             const el=i.el;
             if(!isMobile) { 
                el.title = i.event.title + "\\n" + (i.event.extendedProps.description || "");
             }
          },
          eventDrop:async i=> {
              if(!canWrite) return i.revert();
              fetch('/api/events/'+i.event.id,{method:'PUT',body:JSON.stringify({title:i.event.title,description:i.event.extendedProps.description,start_time:i.event.start.toISOString(),end_time:i.event.end?i.event.end.toISOString():i.event.start.toISOString(),color:i.event.backgroundColor})})
          }
        }); calendar.render();
        form.addEventListener('submit',async e=>{ e.preventDefault(); const d=Object.fromEntries(new FormData(e.target)); if(new Date(d.start)>=new Date(d.end)){alert('Waktu selesai harus lebih besar');return} const url=d.id?'/api/events/'+d.id:'/api/events'; const m=d.id?'PUT':'POST'; await fetch(url,{method:m,body:JSON.stringify({title:d.title,description:d.description,start_time:d.start,end_time:d.end,color:d.color})}).then(r=>{if(r.ok){calendar.refetchEvents();closeModal()}else alert('Error')}) })
      });
    </script>
  `, 'Kalender', user));
});
