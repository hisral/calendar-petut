...
app.get('/cashflow', async (c) => {
  ...
  // Cek Hak Akses (Admin/Contributor = true, View Only = false)
    ...
    <div class="h-full flex flex-col bg-slate-50">
      <div class="bg-white border-b border-gray-200 px-4 md:px-8 py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm">
        ...
        <div class="w-full md:w-auto flex flex-col md:flex-row gap-3 items-end md:items-center">
            ...
            <!-- HANYA TAMPIL JIKA BUKAN VIEW ONLY -->
            ${isWriter ? html`
                ...
                <tbody class="divide-y divide-gray-100 text-xs md:text-sm bg-white">
                    ...
                    <tr class="hover:bg-slate-50 transition group">
                        ...
                        <td class="px-4 md:px-6 py-3 text-center">
                            <!-- TOMBOL DELETE HANYA UTK WRITER -->
                            ${isWriter ? html`
...
app.post('/api/cashflow', async (c) => { 
    ...
    if(!s || !canWrite(s)) return c.text('Forbidden: View Only', 403); // PROTEKSI
    ...
    await c.env.DB.prepare('INSERT INTO cashflow (transaction_date,description,amount,created_by) VALUES (?,?,?,?)')
        .bind(b.date, b.description, b.type==='OUT'?-Math.abs(a):Math.abs(a), s.username).run(); 
    return c.redirect('/cashflow'); 
});

app.delete('/api/cashflow/:id', async (c) => { 
    const s = await getSession(c); 
    if(!s || !canWrite(s)) return c.json({error:'Forbidden'}, 403); // PROTEKSI
